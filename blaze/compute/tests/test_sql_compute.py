...
def test_str_cat(sep):
    ...
    if sep is None:
        expr = t_str_cat.name.str.cat(t_str_cat.comment)
        expected = """
                   SELECT accounts2.name || accounts2.comment
                   AS anon_1 FROM accounts2
                   """
    else:
        expr = t_str_cat.name.str.cat(t_str_cat.comment, sep=sep)
        expected = """
...
def test_str_cat_chain():
    expr = (t_str_cat.name
            .str.cat(t_str_cat.comment, sep=' -- ')
            .str.cat(t_str_cat.product, sep=' ++ '))
...
def test_str_cat_no_runtime_exception():
    ...
    """
    expr = t_str_cat.comment.str.cat(t.name)
