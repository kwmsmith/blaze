...
@given(dfs=dataframes(col_types=numeric_col_types,
                      min_cols=1,
                      max_cols=1))
def test_broadcast_one_col(dfs):
    ds, dframe = dfs
    dshape = datashape.dshape(ds)
    name = dshape.measure.names[0]
    t = symbol('t', dshape)
    field = t[name]
    bcast = broadcast_collect(expr=(2 * field - field**2))
    result = compute(bcast, dframe)
    tm.assert_series_equal(result, (2 * dframe - dframe**2)[name])

binop_compare = st.sampled_from({operator.eq,
                                 operator.ge,
                                 operator.gt,
                                 operator.le,
                                 operator.lt,
...
binops = st.sampled_from({operator.mul,
                          operator.add,
                          operator.mod}) | binop_compare

@given(dfs=dataframes(col_types=numeric_col_types,
                      min_cols=2),
       op=binops)
def test_broadcast_two_col(dfs, op):
    ds, dframe = dfs
    dshape = datashape.dshape(ds)
    col0, col1 = dshape.measure.names[:2]
    t = symbol('t', dshape)
    e = op(t[col0], t[col1])
    bcast = broadcast_collect(expr=e)
    result = compute(bcast, dframe)
    tm.assert_series_equal(result, op(dframe[col0], dframe[col1]))


@given(dfs=dataframes())
def test_projection(dfs):
    ds, dframe = dfs
    dshape = datashape.dshape(ds)
    cols = dshape.measure.names[::2]
    t = symbol('t', dshape)
    tm.assert_frame_equal(compute(t[cols], dframe),
                          dframe[cols])


@given(dfs=dataframes(min_cols=1), op=binop_compare)
def test_eq(dfs, op):
    ds, dframe = dfs
    dshape = datashape.dshape(ds)
    col = dshape.measure.names[0]
    t = symbol('t', dshape)
    if dframe.shape[0]:
        val = dframe.loc[0, col]
    else:
        val = 0
    tm.assert_series_equal(compute(op(t[col], val), dframe),
                           op(dframe[col], val))


@given(dfs=dataframes(min_cols=1),
       op=binop_compare)
def test_selection(dfs, op):
    ds, dframe = dfs
    dshape = datashape.dshape(ds)
    col = dshape.measure.names[0]
    t = symbol('t', dshape)
...
### @@@ Everything below has been converted to property testing @@@
